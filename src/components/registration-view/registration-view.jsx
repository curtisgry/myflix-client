...
export default function RegistrationView() {
        ...
        const checkNotEmptySetError = (keyName, message, isReq) => {
                if (!userInfo[keyName]) {
                        setValidationErrors((prevErrors) => ({ ...prevErrors, [`${keyName}Err`]: message }));
                        return false;
                }
                // Reset error when requirements are met
                setValidationErrors((prevErrors) => ({ ...prevErrors, [`${keyName}Err`]: '' }));
        ...
        const checkValidEmail = (isReq) => {
                ...
                if (!isValid) {
                        setValidationErrors((prevErrors) => ({ ...prevErrors, EmailErr: 'Not a valid email' }));
                }
        ...
        return (
                <Form onSubmit={handleSubmit}>
                        <Form.Group controlId="formUsername">
                                ...
                                {validationErrors.UsernameErr ? <span>{validationErrors.UsernameErr}</span> : ''}
                        </Form.Group>

                        <Form.Group controlId="formPassword">
                                <Form.Label>Password:</Form.Label>
                                <Form.Control name="Password" type="password" onChange={handleChange} />
                                {validationErrors.PasswordErr ? <span>{validationErrors.PasswordErr}</span> : ''}
                        </Form.Group>
                        <Form.Group controlId="formEmail">
                                <Form.Label>Email:</Form.Label>
                                <Form.Control name="Email" type="email" onChange={handleChange} />
                                {validationErrors.EmailErr ? <span>{validationErrors.EmailErr}</span> : ''}
                        </Form.Group>
                        <Form.Group controlId="formBirthday">
                                <Form.Label>Date of Birth:</Form.Label>
                                <Form.Control name="Birthday" type="date" onChange={handleChange} />
                                {validationErrors.BirthdayErr ? <span>{validationErrors.BirthdayErr}</span> : ''}
                        </Form.Group>
